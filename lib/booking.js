...
exports.findOne = function (eventId, callback) {
  ...
  let q = `SELECT events.id, venue_id, events.name, description,
    description_extended, thumbnail, event_date, venues.name as venue_name
    FROM events
  ...
  connection.query(q, (err, event) => {
    ...
    */
    ...
    let q = `SELECT tickets.id AS ticket_id, vrow, vseat
      FROM tickets INNER JOIN seats ON seats.id=tickets.seat_id
      WHERE order_id is NULL AND event_id = ${connection.escape(eventId)}
